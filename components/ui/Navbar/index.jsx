...
const Navbar = () => {
    ...
    return (
        <header>
            <nav className={`w-full md:text-sm transition-all duration-400 ${isScrolled ? 'fixed top-0 left-0 z-50 bg-white/75 backdrop-blur-md shadow-sm' : 'bg-white'}`}>
                <div className="custom-screen items-center mx-auto md:flex">
                    ...
                    <div className={`flex-1 pb-3 mt-8 md:pb-0 md:mt-0 md:block ${state ? "" : "hidden"}`}>
                        <ul className="text-black justify-end items-center gap-10 space-y-6 md:flex md:space-x-6 md:space-y-0 md:text-black md:font-medium">
                            {
                                navigation.map((item, idx) => {
                                    return (
                                            <li key={idx} className="duration-150 hover:text-gray-600">
                                                <Link
                                                    href={item.path}
                                                    className="block"
                                                >
